...
import 'package:firebase_ui_auth/src/widgets/internal/title.dart';
import 'package:firebase_ui_auth/src/widgets/internal/universal_button.dart';
import 'package:firebase_ui_auth/src/widgets/internal/universal_scaffold.dart';

import 'package:firebase_ui_auth/src/screens/internal/responsive_page.dart';
...
/// {@endtemplate}
class EmailVerificationScreenCustom extends StatelessWidget {
  ...
  /// EmailVerificationScreenCustom could invoke these actions:
  ///
  /// * [AuthCancelledAction]
  /// * [EmailVerifiedAction]
  ///
  /// ```dart
  /// EmailVerificationScreenCustom(
  ///   actions: [
  ///     EmailVerified(() {
  ...
  const EmailVerificationScreenCustom({
    Key? key,
    this.auth,
    this.actions = const [],
    this.headerBuilder,
      ...
      child: UniversalScaffold(
        body: ResponsivePage(
          ...
          child: Padding(
            ...
            child: _EmailVerificationScreenCustomContent(
              auth: auth,
...
class _EmailVerificationScreenCustomContent extends StatefulWidget {
  /// {@macro ui.auth.auth_controller.auth}
  final FirebaseAuth? auth;
  final ActionCodeSettings? actionCodeSettings;

  const _EmailVerificationScreenCustomContent({
    Key? key,
    required this.auth,
  ...
  @override
  State<_EmailVerificationScreenCustomContent> createState() =>
      __EmailVerificationScreenCustomContentState();
}

class __EmailVerificationScreenCustomContentState
    extends State<_EmailVerificationScreenCustomContent> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        const Align(
          child: Title(text: 'Verifique seu e-mail'),
        ),
        const SizedBox(height: 32),
        const Text(
          'Um email de verificação foi enviado para '
          'o email cadastrado. Acesse sua caixa de '
          'entrada e clique no link enviado.',
